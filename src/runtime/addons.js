...
function _buildDownloadUrl(sourceUrl, proxyBaseUrl) {
  if (!proxyBaseUrl) return sourceUrl;
  const source = new URL(sourceUrl);
  if (source.hostname === "github.com") {
    const match = source.pathname.match(/^\/([^/]+)\/([^/]+)\/(?:releases\/download|archive)\/(.+)$/iu);
...
async function fetchBytes(url) {
  const directFetch = globalThis.__playgroundOriginalFetch || globalThis.fetch.bind(globalThis);
  const response = await directFetch(url, { redirect: "follow" });
  if (!response.ok) throw new Error(`Failed to fetch ${url}: ${response.status}`);
  return new Uint8Array(await response.arrayBuffer());
}

async function fetchPluginArchive(url, _name) {
...
function buildBlueprintMaterializationState({ manifestVersion, blueprintHash, pluginResults, seedSummary }) {
  return { manifestVersion, blueprintHash, materializedAt: new Date().toISOString(), plugins: pluginResults, seed: seedSummary };
}
...
use FacturaScripts\\Core\\Base\\Plugin;
...
async function runPhpScript({ php, scriptPath, scriptContents }) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`PHP script ${scriptPath} failed with status ${response.status}: ${text}`);
    return text;
  } finally {
...
async function installAndActivatePlugins({ php, fsRoot, publish, blueprint }) {
  const pluginResults = [];
  ...
  for (const plugin of blueprint.plugins) {
    ...
    let zipPath = null;
    const requiresDownload = plugin.source?.type === "url" && (!plugin.name || !(await pathExists(php, targetDir)));
    ...
    } else if (plugin.source?.type === "bundled" && !(await pathExists(php, targetDir))) {
      throw new Error(`Blueprint plugin "${plugin.name}" is not available in the bundled runtime and no ZIP URL was provided.`);
    }

    pluginResults.push({ name: plugin.name, zipPath, shouldEnable: plugin.state === "activate", source: plugin.source });
  }

  if (!pluginResults.length) return pluginResults;

  await php.writeFile(BLUEPRINT_PAYLOAD_PATH, encoder.encode(JSON.stringify({ plugins: buildPluginOps(pluginResults) }, null, 2)));
  const text = await runPhpScript({ php, scriptPath: `${fsRoot}/_playground_blueprint_plugins.php`, scriptContents: buildPluginScript({ fsRoot, payloadPath: BLUEPRINT_PAYLOAD_PATH }) });
  const result = JSON.parse(text);
  if (!result?.ok) throw new Error(`Plugin blueprint execution failed: ${text}`);

  return result.plugins || pluginResults.map((entry) => ({ name: entry.name, installed: Boolean(entry.zipPath), enabled: entry.shouldEnable }));
}
...
async function applySeedData({ php, fsRoot, publish, blueprint }) {
  ...
  await php.writeFile(BLUEPRINT_PAYLOAD_PATH, encoder.encode(JSON.stringify(seedPayload, null, 2)));
  const text = await runPhpScript({ php, scriptPath: `${fsRoot}/_playground_blueprint_seed.php`, scriptContents: buildSeedScript({ fsRoot, payloadPath: BLUEPRINT_PAYLOAD_PATH }) });
  const result = JSON.parse(text);
  if (!result?.ok) throw new Error(`Blueprint seed execution failed: ${text}`);
  return result;
}

export async function materializeBlueprintAddons({ php, blueprint, fsRoot, publish, manifestVersion }) {
  const blueprintFingerprint = JSON.stringify({ plugins: blueprint.plugins || [], seed: buildSeedPayload(blueprint) });
  ...
  const pluginResults = await installAndActivatePlugins({ php, fsRoot, publish, blueprint });
  const seedSummary = await applySeedData({ php, fsRoot, publish, blueprint });

  const nextState = buildBlueprintMaterializationState({ manifestVersion, blueprintHash, pluginResults, seedSummary });
  await php.writeFile(BLUEPRINT_STATE_PATH, encoder.encode(JSON.stringify(nextState, null, 2)));
